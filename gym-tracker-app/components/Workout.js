...
function Workout({
    ...
    const [finishConfirmModalVisible, setFinishConfirmModalVisible] =
        useState(false);
    ...
    useEffect(() => {
        setExercises(cloneDeep(workout.exercises) || []);
        ...
        setExercises((prevExercises) => {
            ...
            if (direction === -1 && exerciseIndex > 0) {
                ...
                [
                    ...
                    updatedExercises[exerciseIndex - 1],
                ] = [
                    updatedExercises[exerciseIndex - 1],
                    updatedExercises[exerciseIndex],
                ];
    ...
    const addExecise = (name, weekIndex, workoutIndex) => {
        const updatedExercises = [
            ...
            {
                name: name,
                sets: [{ reps: UIconstants.DEFAULT_REPS, weight: 0 }],
            },
        ];
        setExercises(updatedExercises);
        if (programView) {
    ...
    const handleDeleteExercise = (exerciseIndex, weekIndex, workoutIndex) => {
        const updatedExercises = exercises.filter(
            (exercise, iExercise) => iExercise !== exerciseIndex
        );
        setExercises(updatedExercises);
            ...
            iExercise == exerciseIndex
                ? {
                      ...
                      sets: exercise.sets.map((set, iSet) =>
                          iSet == setIndex
                              ? updateSetValue(set, field, action, value)
                              : set
                      ),
                  }
                : exercise
            ...
            >
                {editable && !programView && (
                    <View style={styles.WorkoutHeader}>
                        <Text style={[styles.boldText, { paddingLeft: 5 }]}>
                            Active Workout
                        </Text>
                        <View
                            style={[
                        ...
                        >
                            ...
                            {workout.saved && (
                                <Text style={styles.normalText}>
                                    {' '}
                                    {workout.saved.toLocaleDateString(
